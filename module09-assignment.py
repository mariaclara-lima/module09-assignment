...
top_categories_by_region = region_category_sales.groupby(level=0).idxmax().apply(lambda x: x[1])
...
for category in avg_units_by_category.index:
    print(
        f"{category}: Avg Units: {avg_units_by_category[category]:.1f}, "
        f"Avg Price: ${avg_price_by_category[category]:,.2f}"
    )
...
print("1. Increase focus on smartphones because they generate the strongest overall product revenue.")
print("2. Prioritize North America and Europe for high-revenue strategies since they lead total regional sales.")
print("3. Review promotion strategy because non-promoted items had a higher average sale value than promoted items.")